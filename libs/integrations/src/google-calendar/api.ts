...
const { client_id, client_secret } = JSON.parse(process.env.GOOGLE_API_CREDENTIALS).web;
...
export const add = async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === "GET") {
    ...
    const state: IntegrationOAuthCallbackState = req.query.state !== "string" ? undefined : JSON.parse(req.query.state as string);
