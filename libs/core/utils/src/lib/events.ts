...
export const getUid = (calEvent: CalendarEvent): string => calEvent.uid ?? translator.fromUUID(uuidv5(JSON.stringify(calEvent), uuidv5.URL));
export const getCancelLink = (calEvent: CalendarEvent): string => `${WEBAPP_URL}/cancel/${getUid(calEvent)}`;
